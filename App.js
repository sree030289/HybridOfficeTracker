...
export default function App() {
  ...
  const scheduleOfficeDayReminders = async () => {
    ...
    // Only schedule for future office days (up to 30 days ahead)
      ...
      if (plannedDays[dateStr] === 'office') {
        ...
        if (planDate > today) {
          await Notifications.scheduleNotificationAsync({
            ...
            trigger: {
              date: new Date(planDate.getFullYear(), planDate.getMonth(), planDate.getDate(), 7, 0)
            },
          });
        }
      }
...
Generated by OfficeTracker - Your Hybrid Work Companion`;
  ...
  const renderHomeScreen = () => {
    ...
    return (
      <ScrollView style={styles.homeContainer} showsVerticalScrollIndicator={false}>
        <Text style={styles.homeTitle}>📅 {dayName}</Text>
          ...
          {weekDays.map((date, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => handlePlannerDayPress(dateStr)}
              >
  ...
  const handleShareWeeklyPlan = async () => {
    ...
    try {
      if (await Sharing.isAvailableAsync()) {
        const textUri = `${documentDirectory}weekly_plan.txt`;
        await FileSystem.writeAsStringAsync(textUri, shareMessage);
  ...
  const renderPlanScreen = () => {
    return (
      <View style={styles.planContainer}>
        <Text style={styles.planTitle}>📅 Plan Your Office Days</Text>
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            for (let i = 0; i < 7; i++) {
              const date = new Date(startOfWeek);
              date.setDate(startOfWeek.getDate() + i + 1); // Start from Monday
              const dateStr = date.toISOString().split('T')[0];
              const dayName = date.toLocaleDateString('en-US', { weekday: 'short' });
            ...
            Alert.alert(
              ...
              [
                ...
                { 
                  ...
                  onPress: () => {
                    // In production, use Clipboard.setString(shareMessage)
                    Alert.alert('📋 Copied!', 'Your weekly plan has been copied to clipboard');
                  }
                },
                { 
                  text: 'Share', 
                  onPress: async () => {
                    if (await Sharing.isAvailableAsync()) {
                      ...
                      Alert.alert('Sharing not available', 'Weekly plan copied to clipboard instead');
                    }
                  }
                }
              ]
          ...
          <View style={styles.headerTop}>
            ...
            <View style={styles.headerButtons}>
              <TouchableOpacity onPress={() => setShowPlanner(true)}>
                <Text style={styles.planButton}>📅{new Date().getDate()} Plan</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                onPress={() => {
                  Alert.alert(
